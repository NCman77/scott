...
class APIManager {
    ...
    async callGemini(apiKey, base64Image) {
        ...
        // 最終優化 Prompt：專殺亂遮+框太小問題
        const prompt = `你是超嚴格的試卷手寫答案偵測專家，只偵測手寫填詞（手寫文字填在填空線上或旁邊）和手寫下劃線標記的詞。

圖片是試卷，嚴格忽略：
- 所有印刷文字（題幹、word bank、標頭、分數、頁碼）
- 填空線本身（_____）
- 任何非手寫標記
  ...
  "masks": [
    {
      ...
      "y": 左上 y (0-1),
      "w": 寬 (0-1，比手寫文字寬 20% 留邊，确保完全蓋住),
      "h": 高 (0-1，比手寫文字高 30% 留邊，确保完全蓋住),
      "reason": "手寫填詞或下劃線答案"
    }
  ]
}
...
規則（必須遵守）：
- 只框手寫文字本身（成人工整或兒童歪扭都抓）
- 框要稍大（自動擴大 20-30%），確保完全蓋住手寫字（包含邊緣模糊或歪斜部分）
- 絕對不要框印刷文字、線、標頭、word bank
- 如果沒手寫答案，回傳 {\"masks\": []}
