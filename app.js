...
class APIManager {
    ...
    async callGemini(apiKey, base64Image) {
        // 使用穩定版 Gemini 2.5 Flash（默認）或 Pro（布局理解更強）
        const model = 'gemini-2.5-flash';  // 穩定版，額度充足
        // const model = 'gemini-2.5-pro';  // Pro 版布局理解更強，推薦！（額度稍少但值得）
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`;

        // 最終策略：填空線錨點（先偵線，框寬=線長，框高=蓋手寫）
        const prompt = `你是試卷填空線答案遮罩專家，任務是偵測所有填空線（_____ 、底線、空白欄、horizontal lines），然後只遮罩線上或附近的手寫答案文字。

步驟：
1. 先找到圖片中所有填空線的位置（水平線或空白填空區域）。
2. 對於每個填空線，如果線上/線內/線附近有手寫文字（填詞、數字、音標），產生一個遮罩框。
3. 框的寬度必須精準匹配填空線的長度。
4. 框的高度自動調整，完整蓋住手寫文字（包含歪斜、連筆、上方/下方部分，留 20-30% margin 不露邊）。

嚴格忽略：
- 所有印刷文字、word bank、標頭、分數、頁碼、題幹
- 沒有手寫的填空線（不要框空線）
...
關鍵要求：
- 寬度絕對匹配填空線長度（不要更大或更小）
- 高度要夠大，確保手寫完全蓋住不露邊（兒童歪扭或成人工整都行）
- 如果填空線沒手寫，回傳空陣列或跳過

範例：{\"masks\": [{\"x\": 0.3, \"y\": 0.2, \"w\": 0.15, \"h\": 0.05, \"reason\": \"填空線上手寫答案\"}]}`;
